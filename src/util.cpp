...
{
    boost::filesystem::path pathConfigFile(GetArg("-conf", "linkcoin.conf"));
...
{
    boost::filesystem::path pathPidFile(GetArg("-pid", "linkcoind.pid"));
...
void AddTimeData(const CNetAddr& ip, int64 nTime)
{
    ...
    if (vTimeOffsets.size() >= 5 && vTimeOffsets.size() % 2 == 1)
    {
        ...
        if (abs64(nMedian) < 35 * 60) // Linkcoin: changed maximum adjust to 35 mins to avoid letting peers change our time too much in case of an attack.
        {
            nTimeOffset = nMedian;
        }
        else
